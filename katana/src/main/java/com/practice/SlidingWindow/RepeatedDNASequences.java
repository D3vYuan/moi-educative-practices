    ...
    public Set<String> findRepeatedSequencesV1(String s, int k) {
        ...
        while (end + 1 <= s.length()) {
            ...
            if (frequencies.containsKey(currentSortedString)) {
                // System.out.println(String.format("Processing: Updating %s",
                // currentSortedString));
                int updatedCount = frequencies.get(currentSortedString) + 1;
                frequencies.replace(currentSortedString, updatedCount);
                System.out.println(
                        String.format("Processing: Updating %s to count #%d", currentSortedString, updatedCount));
            } else {
        ...
        // Set<String> repeatedSequences = frequencies.entrySet().stream().filter(e ->
        // e.getValue() > 1).map(e -> e.getKey()).collect(Collectors.toSet());
