        ...
        public void union(int v1, int v2) {
            ...
            if (parent1 != parent2){
                ...
                if (parent1_rank > parent2_rank){
                    System.out.println(String.format("union(%d, %d): adding parent2 (%d) to parent1 (%d)", v1, v2, parent2, parent1));
                    this.parent.set(parent2, parent1);
                    this.rank.set(parent1, parent1_rank + parent2_rank);
                } else if (parent2_rank > parent1_rank) {
                    System.out.println(String.format("union(%d, %d): adding parent1 (%d) to parent2 (%d)", v1, v2, parent1, parent2));
                    this.parent.set(parent1, parent2);
                    this.rank.set(parent2, parent2_rank + parent1_rank);
                } else {
                    System.out.println(String.format("union(%d, %d): adding parent2 (%d) to parent1 (%d)", v1, v2, parent2, parent1));
                    this.parent.set(parent2, parent1);
                    System.out.println(String.format("union(%d, %d): parent1 (%d) ranked up (%d + 1)", v1, v2, parent1, parent1_rank));
        ...
        for (int row = 0; row < rows; row++) {
            for (int col = 0; col < cols; col++) {
                ...
                if (unionFind.getValue(row, col) == '1'){
                    ...
                    System.out.println(String.format("find(%d, %d): found island index @%d", row, col, currentMajorOrder));
