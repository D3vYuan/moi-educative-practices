...
public class CoinChange {
    /**
     * You're given an integer total and a list of integers called coins. The
     * variable coins hold a list of coin denominations, and total is the total
     * amount of money.
     * You have to find the minimum number of coins that can make up the total
     * amount by using any combination of the coins. If the amount can't be made up,
     * return -1. If the total amount is 0, return 0.
     * 
     * Strategy
     ...
     * and if it is greater than 0, return the target value stored at the - ith
     * index of the counter array. Store this value into a separate variable called
     * result.
    ...
    private int calculateMinimumCoins(int[] coins, int remainingTotal, int[] memo) {
        ...
        // This way we either solve a new sub-problem or look up the answer from the
        // table if it is already computed.
        ...
        for (int j = 0; j < coins.length; j++) {
            ...
            System.out.println(String.format("Total: %d | Coin: %d - Returns [%d] minimum coin", remainingTotal,
                    coins[j], result));
            if (result >= 0 && result < minimum) {
                System.out.println(String.format(
                        "Updating: Result (%d) is smaller than current minimum (%d)...replace minimum with new value",
                        result, minimum));
                minimum = 1 + result;
            }
        }
