...
public class CompilationOrder {
     ...
     * and so on).
     * Some classes are dependent on other classes for compilation. For example, if
     * class B extends class A,
     * then B has a dependency on A.
     * Therefore, A must be compiled before B.
     * 
     * Given a list of the dependency pairs, find the order in which the classes
     * should be compiled.
     * 
     * Strategy
     ...
     * [4] Pop from the queue and store the node in a list, let’s call it sorted
     * order.
     * [5] Decrement the in-degrees of the node’s children by 1. If the in-degree of
     * a node becomes 0, add it to the source queue.
     * [6] Repeat until all vertices have been visited. Return the sorted order
     * list.
     */
